...
class LangGraphEconomicAgent:
        ...
        try:
            ...
            if "gdp" in raw_data and "inflation" in raw_data:
                ...
                try:
                    ...
                    state["messages"].append(f"⚠️ Chart export failed: {str(e)}")
            ...
            # Create GDP-specific chart (when only GDP data is available)
                ...
                if gdp_fig:
                    ...
                    try:
                        gdp_fig.savefig(chart_path, dpi=300, bbox_inches='tight', facecolor='white')
                        plt.close(gdp_fig)  # Close figure to free memory
                        chart_paths.append(chart_path)
                    except Exception as e:
                        state["messages"].append(f"⚠️ Chart export failed: {str(e)}")
            ...
            # Create inflation-specific chart (when only inflation data is available)
                ...
                if inflation_fig:
                    ...
                    try:
                        inflation_fig.savefig(chart_path, dpi=300, bbox_inches='tight', facecolor='white')
                        plt.close(inflation_fig)  # Close figure to free memory
                        chart_paths.append(chart_path)
                    except Exception as e:
                        state["messages"].append(f"⚠️ Chart export failed: {str(e)}")
            ...
            if "industry" in raw_data:
                ...
                if industry_fig:
                    ...
                    try:
                        industry_fig.savefig(chart_path, dpi=300, bbox_inches='tight', facecolor='white')
                        plt.close(industry_fig)  # Close figure to free memory
                        chart_paths.append(chart_path)
                    except Exception as e:
                        state["messages"].append(f"⚠️ Chart export failed: {str(e)}")
            ...
            if "market" in raw_data:
                ...
                if market_fig:
                    ...
                    try:
                        market_fig.savefig(chart_path, dpi=300, bbox_inches='tight', facecolor='white')
                        plt.close(market_fig)  # Close figure to free memory
                        chart_paths.append(chart_path)
                    except Exception as e:
                        state["messages"].append(f"⚠️ Chart export failed: {str(e)}")
            ...
            if correlation_fig:
                ...
                try:
                    correlation_fig.savefig(chart_path, dpi=300, bbox_inches='tight', facecolor='white')
                    plt.close(correlation_fig)  # Close figure to free memory
                    chart_paths.append(chart_path)
                except Exception as e:
                    state["messages"].append(f"⚠️ Chart export failed: {str(e)}")
    ...
    def _create_industry_comparison_chart(self, industry_data: Dict[str, Any]) -> plt.Figure:
        """Create professional industry performance comparison chart using matplotlib"""
        try:
            ...
            fig.suptitle('Industry Performance Analysis Dashboard', fontsize=20, fontweight='bold', y=0.95)
            ...
            # 1. Employment Growth Chart (Top Left)
            bars1 = ax1.bar(industry_labels, employment_changes, color=colors, alpha=0.8, edgecolor='white', linewidth=2)
            ax1.set_title('Employment Growth (YoY %)', fontsize=14, fontweight='bold')
            ax1.set_ylabel('Employment Growth (%)', fontsize=12)
            ax1.grid(True, alpha=0.3)
            
            # Add value labels on bars
            for bar, value in zip(bars1, employment_changes):
            ...
            # 2. Wage Growth Chart (Top Right)
            bars2 = ax2.bar(industry_labels, wage_changes, color=[color.replace('AB', '72') for color in colors], 
                           alpha=0.8, edgecolor='white', linewidth=2)
            ax2.set_title('Wage Growth (YoY %)', fontsize=14, fontweight='bold')
            ax2.set_ylabel('Wage Growth (%)', fontsize=12)
            ax2.grid(True, alpha=0.3)
            
            # Add value labels on bars
            for bar, value in zip(bars2, wage_changes):
                height = bar.get_height()
                ax2.text(bar.get_x() + bar.get_width()/2., height + (0.1 if height >= 0 else -0.3),
                        f'{value:.1f}%', ha='center', va='bottom' if height >= 0 else 'top', fontweight='bold')
            
            # 3. Performance Scatter Plot (Bottom Left)
            valid_x = []
            valid_y = []
            valid_labels = []
            valid_colors = []
            ...
            for i, (emp, wage, label, color, size) in enumerate(zip(employment_changes, wage_changes, industry_labels, colors, employment_levels)):
                if emp != 0 or wage != 0:  # Only include if we have some data
                    ...
                    valid_sizes.append(max(size/1000, 50))  # Scale size for visibility
            
            if valid_x:  # Only create scatter plot if we have data
                scatter = ax3.scatter(valid_x, valid_y, c=valid_colors, s=valid_sizes, alpha=0.7, edgecolors='white', linewidth=2)
                ax3.set_title('Industry Performance Metrics', fontsize=14, fontweight='bold')
                ax3.set_xlabel('Employment Growth (%)', fontsize=12)
                ax3.set_ylabel('Wage Growth (%)', fontsize=12)
                ax3.grid(True, alpha=0.3)
                
                # Add labels for each point
                for x, y, label in zip(valid_x, valid_y, valid_labels):
                    ax3.annotate(label, (x, y), xytext=(5, 5), textcoords='offset points', fontweight='bold')
            else:
                ax3.text(0.5, 0.5, 'No data available for performance metrics', 
                        ha='center', va='center', transform=ax3.transAxes, fontsize=12, color='gray')
                ax3.set_title('Industry Performance Metrics', fontsize=14, fontweight='bold')
            
            # 4. Overall Performance Comparison (Bottom Right)
            bars4 = ax4.bar(industry_labels, performance_scores, color=colors, alpha=0.8, edgecolor='white', linewidth=2)
            ...
            fig.text(0.5, 0.02, 'Data Source: Federal Reserve Economic Data (FRED)', 
    ...
    def _create_gdp_analysis_chart(self, gdp_data: Dict[str, Any]) -> plt.Figure:
        """Create professional GDP analysis chart using matplotlib"""
        try:
            # Set style for professional appearance
            plt.style.use('seaborn-v0_8-whitegrid')
            ...
            # Create figure with subplots
            fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(15, 12))
            fig.suptitle('GDP Analysis Dashboard', fontsize=20, fontweight='bold', y=0.95)
            ...
            if "gdp" in gdp_data and gdp_data["gdp"]:
                ...
                dates = gdp_level_data.data.index
                values = gdp_level_data.data.values
                
                ax1.plot(dates, values, color=colors['gdp'], linewidth=3, marker='o', markersize=4)
                ax1.set_title('GDP Level (Billions USD)', fontsize=14, fontweight='bold')
            ...
            if "gdp_growth" in gdp_data and gdp_data["gdp_growth"]:
                ...
                dates = growth_data.data.index
                values = growth_data.data.values
                
                ax2.plot(dates, values, color=colors['gdp_growth'], linewidth=3, marker='o', markersize=4)
                ax2.fill_between(dates, values, alpha=0.3, color=colors['gdp_growth'])
                ax2.set_title('GDP Growth Rate (%)', fontsize=14, fontweight='bold')
            ...
            if "gdp_per_capita" in gdp_data and gdp_data["gdp_per_capita"]:
                ...
                dates = per_capita_data.data.index
                values = per_capita_data.data.values
                
                ax3.plot(dates, values, color=colors['gdp_per_capita'], linewidth=3, marker='o', markersize=4)
                ax3.set_title('GDP Per Capita (USD)', fontsize=14, fontweight='bold')
                ax3.set_ylabel('GDP Per Capita (USD)', fontsize=12)
                ax3.grid(True, alpha=0.3)
            ...
            # 4. GDP Components Analysis (Bottom Right) - Placeholder
            ax4.text(0.5, 0.5, 'GDP Components Analysis\n(Future Enhancement)', 
                    ha='center', va='center', transform=ax4.transAxes, fontsize=14, color='gray')
            ax4.set_title('GDP Components Analysis', fontsize=14, fontweight='bold')
            ax4.set_xticks([])
            ax4.set_yticks([])
    ...
    def _create_inflation_analysis_chart(self, inflation_data: Dict[str, Any]) -> plt.Figure:
        """Create professional inflation analysis chart using matplotlib"""
        try:
            # Set style for professional appearance
            plt.style.use('seaborn-v0_8-whitegrid')
            ...
            # Create figure with subplots
            fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(15, 12))
            fig.suptitle('Inflation Analysis Dashboard', fontsize=20, fontweight='bold', y=0.95)
            ...
            if "cpi" in inflation_data and inflation_data["cpi"]:
                ...
                dates = cpi_data.data.index
                values = cpi_data.data.values
                
                ax1.plot(dates, values, color=colors['cpi'], linewidth=3, marker='o', markersize=4, label='CPI')
                ax1.set_title('Consumer Price Index (CPI)', fontsize=14, fontweight='bold')
                ax1.set_ylabel('CPI Index', fontsize=12)
                ax1.grid(True, alpha=0.3)
                ax1.tick_params(axis='x', rotation=45)
                ax1.legend()
            ...
            if "cpi" in inflation_data and inflation_data["cpi"]:
                ...
                ax2.plot(dates, values, color=colors['cpi'], linewidth=3, marker='o', markersize=4, label='Headline CPI')
            
            if "core_cpi" in inflation_data and inflation_data["core_cpi"]:
                core_cpi_data = inflation_data["core_cpi"]
                dates = core_cpi_data.data.index
                values = core_cpi_data.data.values
                
                ax2.plot(dates, values, color=colors['core_cpi'], linewidth=3, marker='s', markersize=4, label='Core CPI')
            
            ax2.set_title('Core CPI vs Headline CPI', fontsize=14, fontweight='bold')
            ...
            if "pce" in inflation_data and inflation_data["pce"]:
                ...
                dates = pce_data.data.index
                values = pce_data.data.values
                
                ax3.plot(dates, values, color=colors['pce'], linewidth=3, marker='o', markersize=4, label='PCE')
                ax3.set_title('Personal Consumption Expenditures (PCE)', fontsize=14, fontweight='bold')
                ax3.set_ylabel('PCE Index', fontsize=12)
                ax3.grid(True, alpha=0.3)
                ax3.tick_params(axis='x', rotation=45)
                ax3.legend()
            ...
            if "inflation_rate" in inflation_data and inflation_data["inflation_rate"]:
                ...
                dates = inflation_rate_data.data.index
                values = inflation_rate_data.data.values
                
                ax4.plot(dates, values, color=colors['inflation_rate'], linewidth=3, marker='o', markersize=4, label='Inflation Rate')
                ax4.fill_between(dates, values, alpha=0.3, color=colors['inflation_rate'])
                ax4.set_title('Inflation Rate Trends', fontsize=14, fontweight='bold')
                ax4.set_ylabel('Inflation Rate (%)', fontsize=12)
                ax4.grid(True, alpha=0.3)
                ax4.tick_params(axis='x', rotation=45)
                ax4.legend()
                
                # Add Fed target line (2%)
                ax4.axhline(y=2.0, color='red', linestyle='--', alpha=0.7, label='Fed Target (2%)')
                ax4.legend()
    ...
    def _create_market_trends_chart(self, market_data: Dict[str, Any]) -> plt.Figure:
        """Create professional market trends analysis chart using matplotlib"""
        try:
            # Set style for professional appearance
            plt.style.use('seaborn-v0_8-whitegrid')
            ...
            # Create figure with subplots
            fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(15, 12))
            fig.suptitle('Market Trends Analysis Dashboard', fontsize=20, fontweight='bold', y=0.95)
            ...
            if "fed_funds" in market_data and market_data["fed_funds"]:
                ...
                dates = fed_data.data.index
                values = fed_data.data.values
                
                ax1.plot(dates, values, color=colors['fed_funds'], linewidth=3, marker='o', markersize=4, label='Fed Funds Rate')
            
            if "10y_treasury" in market_data and market_data["10y_treasury"]:
                treasury_data = market_data["10y_treasury"]
                dates = treasury_data.data.index
                values = treasury_data.data.values
                
                ax1_twin = ax1.twinx()
                ax1_twin.plot(dates, values, color=colors['treasury_10y'], linewidth=3, marker='s', markersize=4, label='10Y Treasury')
                ax1_twin.set_ylabel('10Y Treasury Rate (%)', fontsize=12, color=colors['treasury_10y'])
                ax1_twin.tick_params(axis='y', labelcolor=colors['treasury_10y'])
                ax1_twin.legend(loc='upper right')
            
            ax1.set_title('Interest Rates & Yield Curve', fontsize=14, fontweight='bold')
            ax1.set_ylabel('Fed Funds Rate (%)', fontsize=12, color=colors['fed_funds'])
            ax1.tick_params(axis='y', labelcolor=colors['fed_funds'])
            ax1.grid(True, alpha=0.3)
            ax1.tick_params(axis='x', rotation=45)
            ...
            if "unemployment" in market_data and market_data["unemployment"]:
                ...
                dates = unemp_data.data.index
                values = unemp_data.data.values
                
                ax2.plot(dates, values, color=colors['unemployment'], linewidth=3, marker='o', markersize=4, label='Unemployment Rate')
                ax2.fill_between(dates, values, alpha=0.3, color=colors['unemployment'])
                ax2.set_title('Unemployment Trends', fontsize=14, fontweight='bold')
            ...
            if "consumer_confidence" in market_data and market_data["consumer_confidence"]:
                ...
                dates = conf_data.data.index
                values = conf_data.data.values
                
                ax3.plot(dates, values, color=colors['consumer_confidence'], linewidth=3, marker='o', markersize=4, label='Consumer Confidence')
                ax3.set_title('Consumer Confidence', fontsize=14, fontweight='bold')
                ax3.set_ylabel('Consumer Confidence Index', fontsize=12)
                ax3.grid(True, alpha=0.3)
                ax3.tick_params(axis='x', rotation=45)
                ax3.legend()
            ...
            if "industrial_production" in market_data and market_data["industrial_production"]:
                ...
                dates = prod_data.data.index
                values = prod_data.data.values
                
                ax4.plot(dates, values, color=colors['industrial_production'], linewidth=3, marker='o', markersize=4, label='Industrial Production')
                ax4.set_title('Industrial Production', fontsize=14, fontweight='bold')
                ax4.set_ylabel('Industrial Production Index', fontsize=12)
                ax4.grid(True, alpha=0.3)
                ax4.tick_params(axis='x', rotation=45)
                ax4.legend()
    ...
    def _create_correlation_heatmap(self, raw_data: Dict[str, Any]) -> plt.Figure:
        """Create professional correlation heatmap of economic indicators using matplotlib/seaborn"""
        try:
            ...
            if len(combined_data) > 1:
                ...
                ax.set_title('Economic Indicators Correlation Matrix', fontsize=18, fontweight='bold', pad=20)
