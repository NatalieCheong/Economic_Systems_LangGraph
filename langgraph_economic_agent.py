...
class LangGraphEconomicAgent:
    ...
    def __init__(self):
        ...
        self.graph = self._create_workflow().compile()
...
def get_economic_analysis_graph():
    """Factory function to create the economic analysis graph for LangGraph Studio"""
    # Create the workflow directly without going through the agent
    workflow = StateGraph(EconomicAnalysisState)
    
    # Create a dummy agent just to get access to the methods
    # Note: This is a workaround for LangGraph Studio
    try:
        config = Config
        config.validate()
        
        # Add nodes directly
        workflow.add_node("data_collection", lambda state: state)  # Simplified for testing
        workflow.add_node("generate_insights", lambda state: state)
        
        # Set entry point
        workflow.set_entry_point("data_collection")
        workflow.add_edge("data_collection", "generate_insights")
        workflow.add_edge("generate_insights", END)
        
        # Studio expects a compiled graph (Graph/CompiledGraph), not a StateGraph definition
        return workflow.compile()
        
    except Exception as e:
        # Fallback: return a minimal working graph
        workflow = StateGraph(EconomicAnalysisState)
        workflow.add_node("start", lambda state: state)
        workflow.set_entry_point("start")
        workflow.add_edge("start", END)
        # Return a minimal compiled graph as fallback
        return workflow.compile()
