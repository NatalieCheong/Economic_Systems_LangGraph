...
class EconomicDataAgent:
    ...
    def create_economic_dashboard(self, economic_data: EconomicData, save_path: str = None) -> str:
        ...
        # Create figure with subplots - fix layout issues
        fig = plt.figure(figsize=(16, 12))
        gs = fig.add_gridspec(3, 2, hspace=0.4, wspace=0.3)
        
        # Main title with analysis type
        analysis_type = economic_data.analysis_type.replace('_', ' ').title()
        fig.suptitle(f'Economic Analysis Dashboard - {analysis_type}', 
                    fontsize=16, fontweight='bold', y=0.95)
        
        # Check if this is industry-specific analysis
        is_industry_analysis = 'industry' in economic_data.analysis_type.lower()
        
        if is_industry_analysis:
            self._create_industry_specific_dashboard(fig, gs, economic_data)
        else:
            self._create_general_economic_dashboard(fig, gs, economic_data)
    ...
    def _create_industry_specific_dashboard(self, fig, gs, economic_data: EconomicData):
        """Create industry-specific dashboard"""
        industry_type = economic_data.analysis_type.replace('_industry_performance', '').replace('_performance', '')
        
        # Plot 1: Industry Employment Trends
        ax1 = fig.add_subplot(gs[0, 0])
        self._plot_industry_employment(ax1, economic_data, industry_type)
        
        # Plot 2: Industry Investment/Production
        ax2 = fig.add_subplot(gs[0, 1])
        self._plot_industry_production(ax2, economic_data, industry_type)
        
        # Plot 3: Performance Heatmap
        ax3 = fig.add_subplot(gs[1, :])
        self._create_indicators_heatmap(ax3, economic_data)
        
        # Plot 4: Data Quality and Trends
        ax4 = fig.add_subplot(gs[2, 0])
        self._create_data_quality_chart(ax4, economic_data.data_quality_report)
        
        ax5 = fig.add_subplot(gs[2, 1])
        self._create_trend_analysis_chart(ax5, economic_data)

    def _plot_industry_employment(self, ax, economic_data: EconomicData, industry_type: str):
        """Plot industry-specific employment data"""
        employment_indicators = [f'{industry_type}_employment', 'software_employment', 'tech_employment']
        ...
        for indicator_id in employment_indicators:
            if indicator_id in economic_data.indicators:
                ...
                if not indicator.data.empty:
                    ax.plot(indicator.data.index, indicator.data['value'], 
                           linewidth=2, label=indicator.name)
                    plotted = True
        
        if plotted:
            ax.set_title(f'{industry_type.title()} Industry Employment', fontweight='bold')
            ax.set_ylabel('Employment Level')
            ax.legend()
            ax.grid(True, alpha=0.3)
        else:
            ax.text(0.5, 0.5, f'No {industry_type} employment data available', 
                   ha='center', va='center', transform=ax.transAxes)
            ax.set_title(f'{industry_type.title()} Industry Employment', fontweight='bold')

    def _plot_industry_production(self, ax, economic_data: EconomicData, industry_type: str):
        """Plot industry-specific production/investment data"""
        production_indicators = [f'{industry_type}_production', 'business_investment', 'industrial_production']
        
        plotted = False
        for indicator_id in production_indicators:
            if indicator_id in economic_data.indicators:
                indicator = economic_data.indicators[indicator_id]
                if not indicator.data.empty:
                    ax.plot(indicator.data.index, indicator.data['value'], 
        ...
        if plotted:
            ax.set_title(f'{industry_type.title()} Industry Production/Investment', fontweight='bold')
            ax.set_ylabel('Index/Value')
            ax.legend()
            ax.grid(True, alpha=0.3)
        else:
            ax.text(0.5, 0.5, f'No {industry_type} production data available', 
                   ha='center', va='center', transform=ax.transAxes)
            ax.set_title(f'{industry_type.title()} Industry Production/Investment', fontweight='bold')
